...
def generate_rti_receipt_pdf(data: dict) -> bytes:
    ...
    # Using compact 20pt margins to guarantee strict 1-page fit
    ...
    style_bold_label = ParagraphStyle('BoldLabel', parent=styles['Normal'], fontSize=8.5, leading=11, fontName='Helvetica-Bold', textColor=colors.HexColor('#475569'))
    style_value = ParagraphStyle('ValueText', parent=styles['Normal'], fontSize=8.5, leading=11.5, textColor=colors.HexColor('#0F172A'))
    ...
    style_reg_val = ParagraphStyle('RegValue', parent=styles['Normal'], fontSize=18, leading=21, fontName='Helvetica-Bold', alignment=TA_CENTER, textColor=colors.HexColor('#10B981'))
    ...
    style_header_title = ParagraphStyle('H1', parent=styles['Normal'], fontSize=14, leading=17, fontName='Helvetica-Bold', alignment=TA_CENTER, textColor=colors.HexColor('#0B1C3F'))
    ...
    style_section_title = ParagraphStyle('SecTitle', parent=styles['Normal'], fontSize=10, leading=12, fontName='Helvetica-Bold', textColor=colors.HexColor('#0B1C3F'))
    ...
    logo_path = os.path.join(os.path.dirname(__file__), '../../../web/public/logo.png')
    logo_img = None
    ...
    header_table = Table(header_table_data, colWidths=[40, 395, 110])
    ...
    story.append(HRFlowable(width="100%", thickness=0.5, color=colors.HexColor('#E2E8F0'), spaceAfter=8, spaceBefore=0))
    ...
    reg_card_table = Table(reg_card_data, colWidths=[545])
    ...
    app_details_table = Table(app_details_data, colWidths=[150, 395])
    ...
    req_details_table = Table(req_details_data, colWidths=[150, 395])
    ...
    next_card_table = Table(next_card_data, colWidths=[181, 181, 181])
    ...
    qr_box_table = Table(qr_box_data, colWidths=[48, 110])
    ...
    footer_table = Table(footer_table_data, colWidths=[360, 185])
